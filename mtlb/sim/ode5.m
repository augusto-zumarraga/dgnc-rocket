function [tspan,Y] = ode5(odefun,tspan,y0,varargin)
...
if nargin > 3 && isstruct(varargin{end}) && isfield(varargin{end}, 'Events')
    event = varargin{end}.Events;
...
for i = 2:N
  ...
  if ~isempty(event) && feval(event, ti, Y(:,i))
      tspan = tspan(1:i);
      Y     = Y(:,1:i);
      break;
  end
